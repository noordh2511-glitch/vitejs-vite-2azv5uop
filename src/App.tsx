...
function App() {
  ...
  const [manualPriceMode, setManualPriceMode] = useState(false); // 🆕 وضع السعر اليدوي
  const [lastInvoice, setLastInvoice] = useState<any>(null);

  // البحث والمتابعة
  const [searchTerm, setSearchTerm] = useState(''); // 🆕 للبحث في الفواتير

  // إدارة الإعدادات والمدخلات
  ...
  useEffect(() => {
    ...
    // جلب الحسابات أول مرة
    fetchFinancials();
    
    // إعداد التحديث الدوري للمالية (كل 30 ثانية مثلاً لضمان التزامن)
    const interval = setInterval(fetchFinancials, 30000); 

    return () => { unsubP(); unsubC(); unsubS(); clearInterval(interval); };
  }, []);

  // 2. 🔔 نظام التنبيه الصوتي للمتأخرين
  ...
  useEffect(() => {
    ...
    const timer = setInterval(checkOverdue, 60000); // فحص كل دقيقة
    return () => clearInterval(timer);
  }, [invoicesList]);


  // --- دوال العمليات (Functions) ---
  ...
  const fetchFinancials = async () => {
    ...
    s_inv.docs.forEach(d => {
      ...
      if (i.orderStatus !== 'ملغية') {
        ...
        const isNew = i.createdAt.toDate() > lastCloseTime; // هل الفاتورة في الشفت الحالي؟
  ...
  const handleSaveInvoice = async () => {
    ...
    const invData = { 
      ...
      deliveryDate: deliveryDate || new Date(Date.now() + 24*60*60*1000).toISOString(), // افتراضي غداً
      items: cart, 
      totalAmount: total, 
  ...
  return (
    <div className="app-container">
      ...
      <style>{`
        ...
        .products-grid { display: grid; grid-template-columns: repeat(auto-fill, minmax(110px, 1fr)); gap: 10px; overflow-y: auto; align-content: start; }
        .prod-btn { background: var(--surface); border: 1px solid #e5e7eb; padding: 10px; border-radius: 12px; cursor: pointer; height: 100px; display: flex; flex-direction: column; align-items: center; justify-content: center; box-shadow: 0 1px 2px rgba(0,0,0,0.05); transition: 0.2s; }
        ...
        .badge { background: #f3f4f6; padding: 2px 8px; border-radius: 6px; font-size: 12px; }
        ...
        <div className="pos-layout no-print">
          ...
          <div style={{overflowY: 'auto'}}>
            <div style={{display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom: 15}}>
              ...
              >
                {manualPriceMode ? '✏️ السعر اليدوي: مفعل' : '🖐️ تفعيل السعر اليدوي'}
              </button>
            </div>
                ...
                <div key={idx} className="cart-item" style={{background: it.isUrgent ? '#fef2f2' : 'transparent'}}>
                  ...
                  <div style={{display:'flex', alignItems:'center', gap:5}}>
                    <button onClick={() => {
                      ...
                      }} style={{background:'#fee2e2', border:'none', width:25, borderRadius:4, cursor:'pointer'}}>-</button>
                    <span>{it.qty}</span>
                    <button onClick={() => setCart(cart.map(c => c.id === it.id ? {...c, qty: c.qty + 1, total: (c.qty+1)*c.price} : c))} 
                      style={{background:'#d1fae5', border:'none', width:25, borderRadius:4, cursor:'pointer'}}>+</button>
                  </div>
      ...
      {/* ==================== 2. شاشة المتابعة (Tracking & Kanban) ==================== */}
      {view === 'tracking' && (
        <div className="no-print" style={{height: '100%', display: 'flex', flexDirection: 'column'}}>
          ...
          <div style={{marginBottom: 15, background: '#fff', padding: 10, borderRadius: 12, display:'flex', gap:10}}>
            <input type="text" placeholder="🔍 بحث (رقم الفاتورة، اسم الزبون، هاتف)..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} style={{margin:0, flex:1}} />
      ...
      {view === 'expenses' && (
        <div className="no-print" style={{maxWidth:'600px', margin:'0 auto'}}>
          <div className="card">
            ...
            <select id="exp-cat" style={{marginBottom:10}}>
            ...
            <button className="btn-main" style={{background:'#ef4444'}} onClick={async () => {
              ...
              if(expName && expAmount) {
                await addDoc(collection(db, 'expenses'), { name: expName, amount: Number(expAmount), category: cat, createdAt: Timestamp.now() });
          ...
          <div className="card">
            <table style={{width:'100%', textAlign:'right'}}>
              ...
              <tbody>{expensesList.map(e => <tr key={e.id}><td>{e.name}</td><td><span className="badge">{e.category}</span></td><td style={{color:'red'}}>{e.amount}</td></tr>)}</tbody>
            </table>
          </div>
        </div>
      )}
                ...
                {products.map(p => (
                  <tr key={p.id} style={{borderBottom:'1px solid #eee'}}>
                    ...
                    <td>
                      ...
                      <button onClick={async()=>{ if(confirm("حذف؟")) await deleteDoc(doc(db,'products',p.id)) }} style={{background:'#ef4444', color:'white', border:'none', borderRadius:4, padding:'2px 8px'}}>حذف</button>
                    </td>
                  </tr>
                ))}
              </tbody>
